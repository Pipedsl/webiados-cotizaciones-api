...
2. El servidor tiene que tener el **correo configurado** (Gmail con App Password — ver
   [`correo-smtp.md`](../correo-smtp.md)).
...
- **No manda recordatorios** automáticos si el cliente no responde.
...
- **"No se pudo enviar el correo (500)":** es configuración de correo, no la cotización. Mira
  [`correo-smtp.md`](../correo-smtp.md) y los logs de Railway (busca `535` o `MailAuthentication`).
  Por diseño, si el correo falla la cotización **no** queda como "Enviada" (no miente).
